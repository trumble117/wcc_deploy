...
# November 5, 2014
#
# Install one-off patches to WCC, WT, and WebLogic
...
$ORACLE_HOME/OPatch/opatch apply -silent -ocmrf $STAGE_DIR/../../responses/ocm.rsp -invPtrLoc /etc/oraInst.loc $STAGE_DIR/PATCHES/p18836021_111180_Generic.zip
...
[[ ! -d $BSU_DIR/cache_dir/FSR2 ]] && mkdir $BSU_DIR/cache_dir/FSR2
unzip -qo $STAGE_DIR/PATCHES/p19182814_1036_Generic.zip -d $BSU_DIR/cache_dir/FSR2
# Sometimes BSU will fail if the patch catalog name is left default
cp $BSU_DIR/cache_dir/FSR2/patch-catalog_21790.xml $BSU_DIR/cache_dir/FSR2/patch-catalog.xml
# BSU won't run unless you run it from it's own directory
cd $BSU_DIR
$BSU_DIR/bsu.sh -install -patch_download_dir=$BSU_DIR/cache_dir/FSR2 -patchlist=FSR2 -prod_dir=$ORACLE_HOME
